...
public class QueryHelper 
{
	...
	{
		T obj = (T)EhCacheEngine.get(fullyQualifiedName, key);
		if(obj == null){
					obj = read(beanClass, sql, params);
		}
		EhCacheEngine.add(fullyQualifiedName, key, obj);
		return obj;	
		  }
	...
	{  
		List<T> obj = (List<T>)EhCacheEngine.get(fullyQualifiedName, key);
		if(obj == null){
	...
	public static <T> List<T> query_slice_cache(String fullyQualifiedName, String key,int cache_obj_count,Class<T> beanClass, String sql, int page, int count, Object...params) {
		...
		List<T> objs = (List<T>)EhCacheEngine.get(fullyQualifiedName, key);
		if(objs == null) {
			objs = query_slice( beanClass, sql, 1, cache_obj_count, params);
			EhCacheEngine.add(fullyQualifiedName, key, objs);
		}
	...
	public static long stat_cache(String fullyQualifiedName, String key,String sql, Object...params) {
		Number value = (Number) EhCacheEngine.get(fullyQualifiedName, key);
		if (value==null) 
		{
			value =stat(sql, params);
			EhCacheEngine.add(fullyQualifiedName, key, value);
		}
